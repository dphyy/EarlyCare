...
6. Open **Care desk** to review next due time, due/overdue status, next-call plan, categorized history, risk scores, transcripts, escalation, and task actions.
7. Use the Care desk **Log missed** action, or `POST /checkins/missed`, when a scheduled call and retry are unanswered; this records the missed attempt and triggers the same schedule, escalation, record, and volunteer-task flow as the demo scenario.
8. Use **Live call** for the live ElevenLabs path when provider credentials are configured in `backend/.env`.

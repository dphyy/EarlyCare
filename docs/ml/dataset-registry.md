...
The machine-readable registry is `research/speech_ml/dataset_registry.json`. Use `research/speech_ml/dataset_registry.py` to inspect local readiness before fetching data or training.
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [NeuroVoz](https://github.com/BYO-UPM/Neurovoz_Dababase) | `access-needed` | Parkinson's speech watch | Public repo and restricted Zenodo dataset record; request/confirm access and license before local use or derived release. | PD vs healthy control. | Castilian Spanish. | Sustained vowels, DDK, listen-and-repeat utterances, monologue. | Repo describes 108 speakers: 53 Parkinsonian, 55 controls; 2,903 audio files. | Yes, after access. | First target for offline raw-audio embeddings, task design, and no-diagnosis watch scoring once access is approved. |
...
| [UCI Parkinson's Speech with Multiple Types of Sound Recordings](https://archive.ics.uci.edu/dataset/301/parkinson%2Bspeech%2Bdataset%2Bwith%2Bmultiple%2Btypes%2Bof%2BAudio%2Brecordings) | `feature-only` | Parkinson's speech watch | UCI lists the dataset under CC BY 4.0; cite the UCI dataset and original paper. | PD vs healthy control; UPDRS available for PD. | Turkish. | Sustained vowels, numbers, words, short sentences. | 20 PD and 20 controls in training set; additional sustained-vowel test set. | No, extracted feature tables. | Feature-level smoke tests through `convert_feature_table.py`, not raw embedding validation. |
...
| [Bridge2AI Voice](https://physionet.org/content/b2ai-voice/) | `access-needed` | Broad voice-health validation | PhysioNet/Bridge2AI release tier and controlled raw-audio terms must be reviewed. | Voice, neurological, mood, respiratory, and related clinical metadata depending on release tier. | North American multi-site context. | Standardized voice tasks, derived voice features, questionnaires. | PhysioNet v3.1 describes 833 participants in the low-risk public release. | Controlled-access for raw audio; public release includes lower-risk derivations. | Future validation source, not a quick Parkinson's or concussion training set. |
...
Use NeuroVoz first after access is approved for offline Parkinson's speech-watch validation because it has raw audio, matched control data, and task types that map to the EarlyCare prompts. Use UCI feature datasets only for quick scoring sanity checks. Defer supervised concussion ML until there is an approved acute concussion speech dataset with raw audio, consent terms, and enough validation coverage.
